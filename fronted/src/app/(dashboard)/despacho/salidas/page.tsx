...
import { Plus, Check, Search, Truck, X, Trash2, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function DespachosPage() {
  ...
  // Filtrar despachos en base a búsqueda y fechas
  const filteredDispatches = dispatches.filter(d => {
    const matchesSearch = d.dispatchNumber.toLowerCase().includes(search.toLowerCase()) || 
    ...
    if (startDate || endDate) {
      ...
      if (startDate) {
        ...
        if (dDate < sDate) matchesDate = false;
      }
      if (endDate) {
        const eDate = new Date(endDate);
        eDate.setHours(0, 0, 0, 0);
        if (dDate > eDate) matchesDate = false;
      }
    }
          ...
          <div className="flex flex-wrap gap-3 items-center">
            <div className="relative w-full sm:w-64">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-carey-red text-sm text-gray-900"
                value={search}
          ...
          <div className="flex items-center justify-between bg-white px-6 py-4 border-t border-gray-100">
            <div className="text-sm text-gray-500">
              ...
              <span className="font-medium">
                {Math.min(currentPage * ITEMS_PER_PAGE, filteredDispatches.length)}
              </span>{' '}
              de <span className="font-medium">{filteredDispatches.length}</span> despachos
            </div>
